...
classify digit at end. Same as train-model.py but records model activities/gradients
'''
...
from correlation import get_popcorr, get_pearson_corr, get_pearson_corr_batch, get_coding_space, get_sparsity, get_weight_info, get_pearson_corr_comp, get_pearson_corr_all
...
def train_model(
    ...
    bptt=None, synth_nlayers=0, synth_nhid=None, nfibres=None, bias=None,
    record_grads=False, 
    ...
    if nfibres is not None:
        ...
        #mask = generate_mask(nfibres, weight.shape)
    ...
    setup = setup_training(
        model, validation_data, optim, ['xent', 'acc'], lr, l2_norm,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--bias', type=float, default=None,
                        help='Bias for RNN initialisation')   

    #parser.add_argument('--record-weights-info', action='store_true', help='record basic qualities of the synthetic weights')
        ...
        for i, seed in enumerate(seeds):
            ...
            for j, bias in enumerate(biases):
                ...
                for k, nfibres in enumerate(nfibres_list):
                    ...
                    #run training
                    val_acc, grads, model = train_model(**vars(args))
		    ...
		    #statistics for learnt weights (not modeled in neurips2021)
                    if nfibres != -1:
                        ...
                        for w, weight in enumerate([synthesiser.input_trigger.weight, synthesiser.layers[0].weight, synthesiser.layers[0].bias]):
                            weights_info[i, j, k-1, w, 0] = get_weight_info(weight, absolute=False)
                    ...
                    np.save(savedir + 'scores.npy', val_acc)
