...
def train_model(
    ...
    bptt=None, synth_nlayers=0, synth_nhid=None, nfibres=None, record_grads=False, 
    ablation_epoch=-1, fixed_synth=False, synth_ablation_epoch=-1,
    ...
    if spars_int is not None:
        name_helper = "T{}_sp{}_seed{}".format(bptt, spars_int, seed)
        if draw_digits:
            name_helper = "dd_" + name_helper
        if model == 'LSTM':
            name_helper += 'lstm'
        else:
            name_helper += 'dni'
    ...
    print("Ablation epoch set at", ablation_epoch)
    print("synth Ablation epoch set at", synth_ablation_epoch)
    ...
    setup = setup_training(
        ...
        bptt, batch_size, False, record_grads, spars_int, mask=mask,
        fixed_synth=fixed_synth,
        synth_ablation=synth_ablation
    )
    ...
    if not record_grads:
        ...
        return train_mse, val_mse, grads, model
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input-size', type=int, default=28,
        help='the default dimensionality of each input timestep (28 = 1 row).'
    ...
    all_scores = np.zeros((len(seeds), len(models), 2, args.epochs)) #3 for training_mse, validation_mse  
    ...
    for i, s in enumerate(seeds):
        ...
        for j, model in enumerate(models):
            ...
            train_mse, val_mse = train_model(**vars(args))
    ...
    if args.draw_digits:
        ...
        root = os.path.join(root, 'fixedmag/')
        vecname = "fixedmag_" + vecname  
    
    if args.nfibres is not None and args.model in ['DNI_LSTM', 'DNI_TANH']:
    ...
    print("Saved array as {}".format(root+vecname))
